...
class Network(nn.Module):
  ...
  def _add_losses(self, sigma_rpn=3.0):
    ...
    roi_labels, roi_weights ,keep_inds = self.get_refine_supervision(det_cls_product, self._image_gt_summaries['ss_boxes'],
                                                                     self._image_gt_summaries['mcg_ss_ind'],
  ...
  def get_refine_supervision(self, refine_prob, ss_boxes, mcg_ss_ind, image_level_label):
      '''
      ...
      ###ruby###
      mcg_ind = np.where(mcg_ss_ind == 0)[0]
      ####
      for i in range(self._num_classes):
          if image_level_label[0, i] == 1:
              ...
              ###ruby###
              cls_prob_tmp = cls_prob[mcg_ind, i]
              ####
  ...
  def forward(self, image, image_level_label ,im_info, gt_boxes=None, ss_boxes=None, mcg_ss_ind=None, mode='TRAIN'):
    ...
    ###ruby###
    self._image_gt_summaries['mcg_ss_ind'] = mcg_ss_ind
    ####
  ...
  def get_summary(self, blobs):
    self.eval()
    self.forward(blobs['data'], blobs['image_level_labels'], blobs['im_info'], blobs['gt_boxes'], blobs['ss_boxes'], blobs['mcg_ss_ind'])
    self.train()
