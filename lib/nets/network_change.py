...
class Network(nn.Module):
  ...
  def get_refine_supervision(self, refine_prob, ss_boxes, image_level_label):
      ...
      thresh_value_for_iteration = 0.9
      ...
      chosen_pgt = np.zeros((0, 4), dtype = np.float32)
      weights = np.zeros((refine_prob.shape[0], 1), dtype = np.int32)
      chosen_inds = np.zeros((1,0))

      exist_classes = np.where(image_level_label[0] == 1)[0]

      # for foreground
      for per_class in exist_classes:

          chosen_boxes = np.zeros((1,0))

          cls_prob_tmp = cls_prob[:, per_class].copy()
          ss_boxes_tmp = ss_boxes.copy()
          chosen_pgt = np.concatenate((chosen_pgt, ss_boxes_tmp[np.argmax(cls_prob_tmp), 1:].reshape(1, -1)), axis=0)

          per_class_pgt = ss_boxes_tmp[np.argmax(cls_prob_tmp), 1:].reshape(1, -1)
          weights_value = cls_prob_tmp[np.argmax(cls_prob_tmp)]
          overlaps = bbox_overlaps(ss_boxes_tmp[:,1:], per_class_pgt)
          chosen_boxes = np.hstack((chosen_boxes, np.array([np.where(overlaps > cfg.TRAIN.MIL_FG_THRESH)[0]]))).astype(np.int)
          chosen_inds = np.hstack((chosen_inds, np.array([np.where(overlaps > cfg.TRAIN.MIL_FG_THRESH)[0]]))).astype(np.int)

          while 1:
              cls_prob_tmp[chosen_boxes] = 0.0

              if np.max(cls_prob_tmp) >= thresh_value_for_iteration:
                  # print('choosen more samples')
          ...
          chosen_boxes = np.array(list(set(chosen_boxes[0])))
          weights[chosen_boxes] = weights_value
          roi_labels[chosen_boxes,per_class+1] = 1
          roi_labels[chosen_boxes, 0] = 0

      chosen_inds = np.array(list(set(chosen_inds[0])))
      # end

      # for bachground
      overlaps = bbox_overlaps(ss_boxes[:,1:], chosen_pgt)
      ...
      keep_inds = np.concatenate([chosen_inds, bg_inds])
  ...
  def cover(self, box, boxes):

    cover_or_not = np.zeros((0, 1))

    for i in range(boxes.shape[0]):

      if boxes[i,0] <= box[0, 0] and boxes[i,1] <= box[0, 1] and boxes[i,2] >= box[0, 2] and boxes[i,3] >= box[0, 3]:
        cover_or_not[i,0] = np.vstack((cover_or_not, np.array([[i]])))
