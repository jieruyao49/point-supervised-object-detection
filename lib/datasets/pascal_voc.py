...
class pascal_voc(imdb):
  ...
  def _load_selective_search_roidb(self, gt_roidb):
    ...
    ###ruby###
    raw_mcg_ss_ind = sio.loadmat(filename)['mcg_ss_ind'].ravel()
    mcg_ss_ind_list= []
    ####
    box_list = []
    for i in range(raw_data.shape[0]):
        ...
        boxes = boxes[keep, :]
        ###ruby###
        mcg_ss_ind = raw_mcg_ss_ind[i]
        mcg_ss_ind = mcg_ss_ind[keep]
        ####
        ...
        ###ruby###
        mcg_ss_ind = mcg_ss_ind[keep]
        mcg_ss_ind_list.append(mcg_ss_ind)
        ####

    ###ruby###
    # mcg_point_raw_data = sio.loadmat(filename)['boxes_mcg_point'].ravel()
    # mcg_point_box_list = []
    # for i in range(mcg_point_raw_data.shape[0]):
    #     boxes = mcg_point_raw_data[i][:, (1, 0, 3, 2)] - 1
    #     keep = ds_utils.unique_boxes(boxes)
    #     boxes = boxes[keep, :]
    #     keep = ds_utils.filter_small_boxes(boxes, self.config['min_size'])
    #     boxes = boxes[keep, :]
    #     mcg_point_box_list.append(boxes)
    ###

    return self.create_roidb_from_box_list(box_list, gt_roidb, mcg_ss_ind_list)
